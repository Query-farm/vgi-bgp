...
# version the worker advertises over VGI (`bgp_version()`) and the uploaded
...
# Read `version = "..."` from the [workspace.package] table only (avoids matching
# rust-version or a dependency's version line).
VERSION="$(awk '

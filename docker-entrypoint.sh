...
#
# Dispatch the single vgi-bgp image into one of its transports:
...
#
# The worker is stateless (it decodes MRT archives handed to it per call), so
# there is no /data to create and no state env to wire — each mode just exec's
# the binary. The catalog name defaults to `bgp` inside main().
set -e
...
case "${1:-http}" in
  http)
    ...
    exec bgp-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec bgp-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec bgp-worker "$@"

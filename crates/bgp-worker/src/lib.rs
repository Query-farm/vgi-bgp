...
//! the `bgp-wasm` crate (the browser build, which serves the same `Worker` over
...
/// The catalog name DuckDB sees in `ATTACH 'bgp' (TYPE vgi, …)`. Defaults to
/// `bgp`, but honors an explicit override so a test harness can rename it. Also
